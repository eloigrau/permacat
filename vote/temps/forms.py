...
from .models import Suffrage_ouinon, Vote_ouinon, Commentaire
...
class Suffrage_ouinon_form(forms.ModelForm):
    ...
    class Meta:
        model = Suffrage_ouinon
        fields = [ 'asso', 'question', 'description',  'estAnonyme', 'start_time', 'end_time']
        widgets = {
    ...
    def save(self, userProfile):
        instance = super(Suffrage_ouinon_form, self).save(commit=False)

        max_length = Suffrage_ouinon._meta.get_field('slug').max_length
        instance.slug = orig = slugify(instance.question)[:max_length]

        for x in itertools.count(1):
            if not Suffrage_ouinon.objects.filter(slug=instance.slug).exists():
                break
...
SuffrageFormset = formset_factory(Suffrage_ouinon_form, extra=1)

class Suffrage_ouinon_ChangeForm(forms.ModelForm):

    class Meta:
        model = Suffrage_ouinon
        fields = ['asso', 'description', 'start_time', 'end_time', 'estAnonyme',  'estArchive']
        widgets = {
            'contenu': SummernoteWidget(),
    ...
    def save(self, userProfile):
        instance = super(Suffrage_ouinon_ChangeForm, self).save(commit=False)
        instance.save(userProfile)
        return instance
...
class Vote_ouinon_form(forms.ModelForm):
    class Meta:
        model = Vote_ouinon
    ...
    def save(self, suffrage, userProfile):
        instance = super(Vote_ouinon_form, self).save(commit=False)
...
class Vote_ouinon_ChangeForm(forms.ModelForm):
    class Meta:
        model = Vote_ouinon
        exclude = ['suffrage','auteur']
        widgets = {
