...
from .factories import LANDSCAPE_IMAGE_PATH, QUOTING_IMAGE_PATH, \
    UNICODE_IMAGE_PATH, NONSENSE_IMAGE_PATH, AlbumFactory, PhotoEffectFactory, PhotoFactory
...
class PreviousNextTest(PhotologueBaseTest):
    """Tests for the methods that provide the previous/next photos in a album."""
    ...
    def setUp(self):
        ...
        self.test_album.photos.add(self.pl1)
        self.test_album.photos.add(self.pl2)
        self.test_album.photos.add(self.pl3)
    ...
    def test_previous_simple(self):
        # Previous in album.
        self.assertEqual(self.pl1.get_previous_in_album(self.test_album),
                         None)
        self.assertEqual(self.pl2.get_previous_in_album(self.test_album),
                         self.pl1)
        self.assertEqual(self.pl3.get_previous_in_album(self.test_album),
                         self.pl2)
    ...
    def test_previous_public(self):
        ...
        self.assertRaisesMessage(ValueError,
                                 ...
                                 self.pl2.get_previous_in_album,
                                 self.test_album)
        self.assertEqual(self.pl3.get_previous_in_album(self.test_album),
                         self.pl1)

    def test_previous_album_mismatch(self):
        """Photo does not belong to the album."""
        self.pl4 = PhotoFactory()

        self.assertRaisesMessage(ValueError,
                                 'Photo does not belong to album.',
                                 self.pl4.get_previous_in_album,
                                 self.test_album)
    ...
    def test_next_simple(self):
        # Next in album.
        self.assertEqual(self.pl1.get_next_in_album(self.test_album),
                         self.pl2)
        self.assertEqual(self.pl2.get_next_in_album(self.test_album),
                         self.pl3)
        self.assertEqual(self.pl3.get_next_in_album(self.test_album),
                         None)
    ...
    def test_next_public(self):
        ...
        self.assertRaisesMessage(ValueError,
                                 'Cannot determine neighbours of a non-public photo.',
                                 self.pl2.get_next_in_album,
                                 self.test_album)
        self.assertEqual(self.pl3.get_next_in_album(self.test_album),
                         None)

    def test_next_album_mismatch(self):
        """Photo does not belong to the album."""
        self.pl4 = PhotoFactory()

        self.assertRaisesMessage(ValueError,
                                 'Photo does not belong to album.',
                                 self.pl4.get_next_in_album,
                                 self.test_album)
