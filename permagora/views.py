...
def risques(request):
    dico_risques = [
        ("Ressources en berne :", "<ol><li>l'eau en danger: nappes phréatiques en net recul, pollués et salinisées, précipitations en forte diminution, assèchement des cours d'eau et des sols, </li><li> les sols pollués/détruits par l'agro-industrie, </li><li> l'approvisionnement énergétique en danger (principalement le pétrole), \
        ...
        ("Une démocratie en berne", "Le système électoral aperdu de sa crédibilité. Il efface une grande partie de la population, et n'est plus adapté aux enjeux actuels. L'interet collectif semble oublié. Les citoyens se sont vus déposséder de leur pouvoir au profit d'un système clanique. IL est temps que les citoyens retrouvent leur juste place dans les décisions qui les concernent."),
...
def preconisations(request):
    ...
    ("Reforestation",
     ...
     ("Gestion des ressources", "limitation prévisionnelle des usages  (industries, transport, domestique) de l'énergie notamment, afin d'anticiper leur diminution d'approvisionnement. Lisser une place aux ressources naturelles sans intervention humaine afin de laisser la biodiversité s'auto réguler et s'adapter au changeent climatique")
    ]
